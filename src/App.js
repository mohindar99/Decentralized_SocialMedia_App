...
import icon from "./assets/follow_icon.png";
import {
  urlClient,
...
function App() {
  ...
  // grabbing the information from blockchain(lens api)

  async function getRecommandedProfiles() {
    const response = await urlClient
      .query(queryRecommendedProfiles)
      .toPromise();
    //taking only the first file from the recommandeed users
  ...
  //for following the user we need to send a transaction
  //we are following the user by id
  ...
  // for segregating the regular urls with ipfs urls
  const parseImageUrl = (profile) => {
    ...
    return "/asserts/rain_background.jpg";
  };
  // for rendering some of the functions in the start of the app
  useEffect(() => {
                ...
                <Box width="75px" height="75px" marginTop="8px">
                  <img
                    ...
                    onError={({ currentTarget }) => {
                      ...
                      currentTarget.src = "./asserts/rain_background.jpg";
                    }}
                  />
                </Box>
                {/* POST CONTENT */}
